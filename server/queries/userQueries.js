...
const UsernameEmailAvailibility = async (usernameEmail, emailOrNot) => {
    try {
        if (emailOrNot) {
...
const registerUser = async (usernameEmail, emailOrNot, password) => {
    try {
        if (!emailOrNot) {
...
const getUser = async (usernameEmail, emailOrNot) => {
    try {
        if (emailOrNot) {
...
const updateTokens = async (usernameEmail, emailOrNot, refreshToken, accessToken) => {
    try {
        if (!emailOrNot) {
            if (refreshToken === "") {
...
const changePassword = async (usernameEmail, emailOrNot, password) => {
    try {
        if (emailOrNot) {
